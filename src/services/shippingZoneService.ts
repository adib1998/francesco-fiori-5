  ...
  private async saveSettings() {
    try {
      ...
      const { error } = await supabase
        .from('settings')
        .upsert({ key: 'shippingZoneSettings', value: this.settings });
      ...
      const { error: zonesError } = await supabase
        .from('settings')
        .upsert({ key: 'deliveryZones', value: this.deliveryZones });

      if (zonesError) throw zonesError;
    } catch (error) {
      console.warn('Failed to save shipping zone settings:', error);
      // Fallback to localStorage
