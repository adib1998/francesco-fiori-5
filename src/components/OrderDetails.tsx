...
const OrderDetails = ({ order, onUpdate, onDelete }: OrderDetailsProps) => {
  ...
  const handleQuickStatusUpdate = async (status: string) => {
    ...
    try {
      ...
      toast({
        ...
        description: `Order #${order.order_number} has been ${status}`,
      });
  ...
  const handleDeleteOrder = async () => {
    ...
    try {
      ...
      toast({
        title: 'Order Deleted',
        description: `Order #${order.order_number} has been permanently deleted`,
      });
      ...
      if (error.message.includes('does not exist')) {
        ...
        errorMessage = 'Cannot delete order - it has related records that need to be removed first';
    ...
    <Card>
      ...
      <CardContent className="space-y-6">
        ...
        <div>
          ...
          <div className="space-y-1 text-sm">
            <div><strong>Total Amount:</strong> {formatCurrency(Number(order.total_amount))}</div>
            ...
            {order.shipped_at && (
              <div><strong>Shipped:</strong> {formatDate(order.shipped_at)}</div>
            )}
            {order.delivered_at && (
              <div><strong>Delivered:</strong> {formatDate(order.delivered_at)}</div>
            )}
          </div>
        </div>
